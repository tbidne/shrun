...
putRegionLogDirect log = do
  commonLogging <- asks @env getCommonLogging
  (consoleLogging, _, _) <- asks (getConsoleLogging @env @rgn)
  mFileLogging <- asks @env getFileLogging
  ...
  MRL.withRegion @rgn Linear $ \rgn -> MRL.logRegion (log ^. #mode) rgn (consoleLog ^. #unConsoleLog)
...
putDebugLogDirect ::
  ...
  LogMessage ->
  Eff es ()
putDebugLogDirect = putDebugLogHelper @env (putRegionLogDirect @env @rgn)
...
putDebugLog ::
  ...
  LogMessage ->
  Eff es ()
putDebugLog = putDebugLogHelper @env (\log -> MRL.withRegion @rgn Linear $ \rgn -> putRegionLog @env @rgn rgn log)
