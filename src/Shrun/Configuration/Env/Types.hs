...
updateCommandStatus command result = do
  commandStatusMap <- asks @env getCommandStatusMap <&> view #unCommandStatusMap
...
getReadCommandStatus = asks @env getCommandStatusMap >>= readCommandStatus
...
setTimedOut ::
  ...
  Eff es ()
setTimedOut = asks @env getHasTimedOut >>= \rgn -> writeTVarA' rgn True
...
whenTimedOut ::
  ...
  Eff es ()
whenTimedOut m = do
  hasTimedOut <- readTVarA' =<< asks @env getHasTimedOut
  when hasTimedOut m
