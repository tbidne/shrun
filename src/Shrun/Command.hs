...
runCommands runner = do
  ...
  Async.mapConcurrently_ (runCommand @env @rgn runner cdg commandStatusMap vtxSemMap) roots
      ...
      case status of
        PredecessorUnfinished depV ->
          Logging.logDebug @env $ \lvl -> do
            logCommandAction @env @rgn cdg lvl prevVertex debugMsg (Just depV) vertex
        PredecessorFailure failExpected depV -> do
          if failExpected
            then
              logCommandAction @env @rgn cdg LevelWarn prevVertex failOkMsg (Just depV) vertex
            else
              logCommandAction @env @rgn cdg LevelError prevVertex errMsg (Just depV) vertex
            ...
            Just mvar ->
              ...
              tryTakeMVar mvar >>= \case
                Nothing ->
                  ...
                  Logging.logDebug @env $ \lvl -> do
                    logCommandAction @env @rgn cdg lvl prevVertex alreadyRunningMsg Nothing vertex
                Just () -> do
                  ...
                  Logging.logDebug @env $ \lvl -> do
                    logCommandAction @env @rgn cdg lvl prevVertex startMsg Nothing vertex
...
logCommandAction cdg lvl mPrevVertex msgFn mDepVertex vertex = do
  ...
  withRegion @rgn Linear $ \rgn -> Logging.putRegionLog @env rgn log
  where
