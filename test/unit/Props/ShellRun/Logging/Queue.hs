...
--
-- @since 0.1.0.0
module Props.ShellRun.Logging.Queue
  ( props,
  )
where
  ...
  TH.testProperty "Starts with timestamp" $
    H.withTests limit $
      H.property $ do
        ...
        let MkLogText result = runMockTime $ Queue.formatFileLog log
  ...
  TH.testProperty "Formats command" $
    H.withTests limit $
      H.property $ do
        ...
        let cmdTxt = "[" <> cmd' <> "]"
            MkLogText result = runMockTime $ Queue.formatFileLog log
        H.annotate $ T.unpack result
        ...
        let MkLogText result = runMockTime $ Queue.formatFileLog log
            expected =
              "["
                ...
                <> Log.logToPrefix log
                <> "["
                <> maybe "test error: did not receive Just command" command cmd
                <> "] "
                <> msg
                <> "\n"
